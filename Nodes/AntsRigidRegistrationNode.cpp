...
AntsRigidRegistrationNode::AntsRigidRegistrationNode(string name, string desc) : AbsNode(name, desc) {
    initControls();
}

void AntsRigidRegistrationNode::initControls() {
    ControlWrapper levelsComposite = CompositeControl(1, "Levels", "配置Rigid配准的层级<降采样率， 迭代次数，平滑半径>，低层级运算速度快，高层级运算精度高", {
